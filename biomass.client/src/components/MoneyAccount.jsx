...
const MoneyAccount = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          openingBalanceAsOf: accountData.openingBalanceAsOf ? new Date(accountData.openingBalanceAsOf).toISOString().split('T')[0] : '',
        ...
        setSnackbar({ open: true, message: response.data.message || 'Failed to load account details', severity: 'error' });
      }
    } catch (error) {
      setSnackbar({ open: true, message: error.message || 'Error loading account details', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditMode && selectedAccount) {
        ...
        response = await axios.put(`${baseUrl}/MoneyAccount/UpdateMoneyAccount/${selectedAccount.moneyAccountId}`, {
          ...
          openingBalanceAsOf: formData.openingBalanceAsOf ? new Date(formData.openingBalanceAsOf) : null,
        ...
        // Create new account
        response = await axios.post(`${baseUrl}/MoneyAccount/CreateMoneyAccount`, {
          accountCode: formData.accountCode,
          name: formData.name,
          kindLookupId: formData.kindLookupId,
          accountHolder: formData.accountHolder,
      ...
      if (response.data.success) {
        setSnackbar({ 
          open: true, 
          message: isEditMode ? 'Money account updated successfully' : 'Money account created successfully', 
          severity: 'success' 
        });
        setOpenDialog(false);
        setSelectedAccount(null);
        ...
        setSnackbar({ open: true, message: response.data.message || 'Failed to save money account', severity: 'error' });
      }
    } catch (error) {
      setError(error.message || 'Error saving money account');
      setSnackbar({ open: true, message: error.message || 'Error saving money account', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setSnackbar({ open: true, message: 'Money account deleted successfully', severity: 'success' });
        setDeleteDialogOpen(false);
        setAccountToDelete(null);
        await loadMoneyAccounts();
      } else {
        setSnackbar({ open: true, message: response.data.message || 'Failed to delete money account', severity: 'error' });
      }
    } catch (error) {
      setSnackbar({ open: true, message: error.message || 'Error deleting money account', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetDefault = async (account) => {
    try {
      ...
      if (response.data.success) {
        setSnackbar({ open: true, message: 'Default account set successfully', severity: 'success' });
        await loadMoneyAccounts();
      } else {
        setSnackbar({ open: true, message: response.data.message || 'Failed to set default account', severity: 'error' });
      }
    } catch (error) {
      setSnackbar({ open: true, message: error.message || 'Error setting default account', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const getAccountTypeColor = (kindLookupId) => {
    switch (kindLookupId) {
      case 6: return 'primary'; // Bank
      case 7: return 'secondary'; // Wallet
      case 8: return 'info'; // Cash
  ...
  const filteredAccounts = moneyAccounts.filter(account =>
    ...
    <Box sx={{ p: 0, width: '100%', backgroundColor: '#f8fffa', minHeight: '100vh' }}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <Typography variant="body2" sx={{ color: '#228B22', fontWeight: 500 }}>
                    Total: {totalCount}
                  </Typography>
                </Box>
              </Grid>
      ...
      >
        <DialogTitle sx={{ 
          ...
          {isEditMode ? 'Edit Money Account' : selectedAccount ? 'View Money Account' : 'Add New Money Account'}
        </DialogTitle>
        <DialogContent sx={{ p: 3 }}>
          <Box sx={{ mt: 1 }}>
            <Typography variant="h6" gutterBottom color="text.secondary">
              {isEditMode ? 'Edit Money Account' : selectedAccount ? 'Money Account Details' : 'Fill in the required information to create a new money account.'}
            </Typography>
            ...
            <Grid container spacing={3} sx={{ mt: 2 }}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth error={!!formErrors.kindLookupId}>
                  ...
                  <Select
                    ...
                    onChange={(e) => setFormData({...formData, kindLookupId: e.target.value})}
                    disabled={!isEditMode && selectedAccount}
                    label="Account Type *"
                    variant="outlined"
                  >
              ...
              {formData.kindLookupId === 7 && (
                <>
                  <Grid item xs={12}>
                    <Typography variant="subtitle1" color="info.main" gutterBottom sx={{ fontWeight: 600, borderBottom: '2px solid #2196f3', pb: 0.5, mt: 2 }}>
                      Wallet Details
                    </Typography>
                  </Grid>
              ...
              <Grid item xs={12}>
                <Typography variant="subtitle1" color="warning.main" gutterBottom sx={{ fontWeight: 600, borderBottom: '2px solid #ff9800', pb: 0.5, mt: 2 }}>
                  Financial Information
                </Typography>
              </Grid>
              ...
              {/* Settings */}
              <Grid item xs={12}>
                <Typography variant="subtitle1" color="primary.main" gutterBottom sx={{ fontWeight: 600, borderBottom: '2px solid #2196f3', pb: 0.5, mt: 2 }}>
                  Settings
                </Typography>
              </Grid>
              ...
              <Grid item xs={12}>
                <Typography variant="subtitle1" color="secondary.main" gutterBottom sx={{ fontWeight: 600, borderBottom: '2px solid #9c27b0', pb: 0.5, mt: 2 }}>
                  User Tracking
                </Typography>
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => setFormData({...formData, createdBy: parseInt(e.target.value) || 0})}
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => setFormData({...formData, updatedBy: parseInt(e.target.value) || 0})}
                  disabled={true} // This should be read-only
                  variant="outlined"
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete the money account "{accountToDelete?.name}"? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ p: 3, gap: 1 }}>
          <Button 
